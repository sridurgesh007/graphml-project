...
def generate_3d_openbabel(smiles):
    try:
        ...
        mol.make3D()        # generate 3D
        mol.localopt()      # optional energy minimization
...
class MoleculeDataset(Dataset):
    def __init__(self, data_path, coords_dir="coords"):
        super().__init__()

        self.coords_dir = coords_dir

        smiles_list = read_smiles(data_path)
        N = 100000 #len(smiles_list)
        print(f"[MolCLR] Loaded {N} SMILES.")
        ...
        for idx in range(N):
            fname = os.path.join(coords_dir, f"{idx:08d}.pt")

            if not os.path.exists(fname):
                missing_files += 1
                continue
            
            coords = torch.load(fname)
    ...
    def __getitem__(self, index):
        global_idx = self.file_indices[index]
        s = self.smiles[index]
        mol = Chem.MolFromSmiles(s)
        mol = Chem.AddHs(mol)

        coord_path = os.path.join(self.coords_dir, f"{global_idx:08d}.pt")
        coords_list = torch.load(coord_path)  # list of [x,y,z]
        
        
                
        for i, item in enumerate(coords_list):
            if item is None or item == [] or (torch.is_tensor(item) and item.numel() == 0):
                print(f"[WARN] coords_list contains None at {coord_path}")
        
        pos = torch.tensor(coords_list, dtype=torch.float)  # (N, 3)
